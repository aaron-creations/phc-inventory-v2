...
import { Calendar as CalendarIcon, Plus, X, User, MapPin, Edit, Trash2, List, LayoutGrid } from 'lucide-react'
...
export default function JobsSection() {
  ...
  function checkTimeConflict(techId, date, startTime, endTime, excludeJobId = null) {
    ...
    return jobs.some(j => {
      ...
      if (j.scheduled_date !== date) return false;
      if (!j.start_time || !j.end_time) return false;
      
      // Overlap occurs if job A starts before job B ends AND job A ends after job B starts
      return j.start_time < endTime && j.end_time > startTime;
    })
  }
  ...
  async function handleCreateJob(e) {
    ...
    if (newJob.is_recurring) {
      ...
      setIsCreating(false)
      setNewJob({ property_id: '', service_type: '', scheduled_date: '', start_time: '', end_time: '', technician_id: '', quoted_price: '', is_recurring: false, frequency: 'monthly', interval_days: '' })
  ...
  async function handleSaveJobEdit(e) {
    ...
    if (editJobForm.technician_id && editJobForm.scheduled_date && editJobForm.start_time && editJobForm.end_time) {
            ...
            <form onSubmit={handleCreateJob} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="time" value={newJob.start_time} onChange={e => setNewJob({...newJob, start_time: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" style={{colorScheme: 'dark'}} />
                </div>
                <div>
                  <label className="block text-white/40 text-[10px] uppercase font-bold tracking-wider mb-1.5">End Time</label>
                  <input type="time" value={newJob.end_time} onChange={e => setNewJob({...newJob, end_time: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" style={{colorScheme: 'dark'}} />
                </div>
              </div>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end mb-4 border-b border-white/5 pb-4">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
            ...
            {displayMode === 'calendar' ? 'Calendar Schedule' : `Upcoming & Active (${upcomingJobs.length})`}
          </h2>
          <div className="flex items-center bg-black/20 rounded-lg p-1 border border-white/5">
            <button 
              onClick={() => setDisplayMode('calendar')}
              className={`p-1.5 rounded flex items-center gap-2 text-xs font-semibold transition-colors ${displayMode === 'calendar' ? 'bg-forest-800 text-white shadow' : 'text-white/40 hover:text-white'}`}
            >
              <CalendarIcon size={14} /> Calendar
            </button>
            <button 
              onClick={() => setDisplayMode('list')}
              className={`p-1.5 rounded flex items-center gap-2 text-xs font-semibold transition-colors ${displayMode === 'list' ? 'bg-forest-800 text-white shadow' : 'text-white/40 hover:text-white'}`}
            >
    ...
    <div className={`p-5 rounded-xl border flex flex-col justify-between ${
      ...
      <div className="flex justify-between items-end pt-3 border-t border-white/5 mt-auto">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-white/80">{job.scheduled_date ? new Date(job.scheduled_date + 'T12:00:00').toLocaleDateString() : 'TBD'}</p>
        </div>
      </div>
    </div>
  )
