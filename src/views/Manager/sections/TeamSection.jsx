            ...
            <div key={tech.id} className="glass rounded-xl overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={e => { e.stopPropagation(); removeTech(tech.id) }}
                    className="text-white/20 hover:text-red-400 text-xs transition-colors px-2 py-1"
                  >
                    Remove
                  </button>
              ...
              {isExpanded && (
                <div className="border-t border-white/5 px-4 pb-4 pt-3">
                  <div className="grid grid-cols-3 gap-3">
                    <StatCard label="This Week" value={s.week?.toFixed(1) ?? '0'} unit="units" color="text-brand-green" />
                    <StatCard label="This Month" value={s.month?.toFixed(1) ?? '0'} unit="units" color="text-brand-blue" />
                    <StatCard label="All-time Logs" value={s.txCount ?? 0} unit="entries" color="text-white/70" />
                  </div>
                </div>
              )}
            </div>
