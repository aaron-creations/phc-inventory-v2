...
export default function UsersSection() {
  ...
  async function denyUser(authUserId, profileId) {
    ...
    try {
      const { error } = await supabase.rpc('delete_user', { target_user_id: authUserId })
      if (error) alert(`Error denying user: ${error.message}`)
    } catch (err) {
      alert(`Unexpected error: ${err.message}`)
    } finally {
      await load()
  ...
  async function linkTechnician(profileId, technicianId) {
    setSaving(profileId)
    await supabase.from('user_profiles').update({ technician_id: technicianId || null }).eq('id', profileId)
    await load()
    setSaving(null)
  }
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <p className="text-white/30 text-xs mb-6">Approve new users and assign roles. Only approved users can access the application.</p>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {saving === p.id ? (
                    ...
                    <>
                      ...
                      <button onClick={() => denyUser(p.user_id, p.id)}
      ...
      <div className="glass rounded-xl overflow-hidden mb-8">
        {loading ? (
          ...
          return (
            <div key={p.id} className={`flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-4 ${i < approvedProfiles.length - 1 ? 'border-b border-white/5' : ''}`}>
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.auth_providers?.map(provider => (
                      <span key={provider} className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 text-white/50 border border-white/10 uppercase tracking-wide">{provider}</span>
                    ))}
                  </div>
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full border flex-shrink-0 ${roleColors[p.role] || 'bg-white/10 text-white/50'}`}>{p.role}</span>
                <button
      ...
      <div className="glass rounded-xl p-4">
        <p className="text-white/30 text-xs mb-4">
          ...
          <a href="https://supabase.com/dashboard/project/jxydfmareguchcqelaiw/auth/providers" target="_blank" rel="noopener noreferrer" className="text-brand-blue hover:underline">Supabase Auth Providers dashboard</a>.
        </p>
        ...
        {inviteMsg && (
          <p className={`text-xs mt-3 ${inviteMsg.startsWith('✅') ? 'text-brand-green' : 'text-brand-orange'}`}>{inviteMsg}</p>
        )}
        <p className="text-white/20 text-xs mt-4">
          Or invite directly from the{' '}
          <a href="https://supabase.com/dashboard/project/jxydfmareguchcqelaiw/auth/users" target="_blank" rel="noopener noreferrer" className="text-brand-blue hover:underline">Supabase Users dashboard</a>
        </p>
      </div>
            ...
            return (
              <div className="bg-forest-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
                <div className="p-4 border-b border-white/10 flex justify-between items-center">
                  ...
                  <button onClick={() => setEditingUserId(null)} className="text-white/40 hover:text-white transition-colors">✕</button>
                </div>
                <div className="p-6 flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={p.role} disabled={saving === p.id || isMe} onChange={e => updateRole(p.id, e.target.value)}
                    ...
                    <div>
                      ...
                      <select value={p.technician_id || ''} disabled={saving === p.id} onChange={e => linkTechnician(p.id, e.target.value)}
                        className="w-full bg-forest-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm outline-none disabled:opacity-40 cursor-pointer">
                        <option value="">No tech link</option>
                        {technicians.map(t => (
                          <option key={t.id} value={t.id}>{t.first_name} {t.last_initial ? `${t.last_initial}.` : ''}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  <div className="border-t border-white/5 pt-4 mt-2">
                    ...
                    <div className="flex flex-col gap-2">
                      {!isMe && hasEmailLogin && p.display_name?.includes('@') && (
                        <button onClick={() => { sendPasswordReset(p.display_name); setEditingUserId(null) }}
                      ...
                      {!isMe && (
                        <button onClick={() => removeUser(p.user_id, p.id)}
