...
import { Plus, Save, X, Wrench } from 'lucide-react'
...
export default function AssetDetail() {
  ...
  async function fetchAssetData() {
    setLoading(true)
    const { data: aData, error: aErr } = await supabase.from('fleet_assets').select('*').eq('id', id).single()
    if (aErr) { console.error(aErr); setLoading(false); return }
    setAsset(aData)
  ...
  async function handleAddLog(e) {
    ...
    if (error) {
      ...
      setLogs([data, ...logs])
      setIsLogging(false)
      setNewLog({ service_type: '', description: '', service_date: new Date().toISOString().split('T')[0], cost: '', next_due_date: '', next_due_miles_hours: '' })
    }
  }
  ...
  async function handleStatusToggle() {
    ...
    if (!error) setAsset({ ...asset, status: newStatus })
  }
  ...
  if (!asset) return <div className="p-8 text-white/40 min-h-screen bg-forest-950">Asset not found.</div>
  ...
  return (
    <div className="min-h-screen bg-forest-950 pb-20">
      <div className="flex bg-black/40 border-b border-white/5 sticky top-0 z-10 px-4 md:px-8 py-4 items-center gap-4">
        <button onClick={() => navigate('/fleet')} className="text-white/50 hover:text-white transition-colors text-sm font-medium">← Back</button>
      </div>
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row gap-4 justify-between md:items-start">
          <div>
            ...
            <h1 className="text-3xl font-serif font-bold text-white mb-2 leading-none">{asset.name}</h1>
        ...
        <section className="bg-forest-900 border border-white/5 rounded-xl overflow-hidden shadow-lg flex flex-col">
          ...
          <div className="p-4 md:p-6 flex-1">
            {isLogging && (
              <div className="mb-8 p-5 bg-black/20 rounded-xl border border-white/10">
                ...
                {logs.map(log => (
                  <div key={log.id} className="p-4 bg-white/[0.02] rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs text-brand-green font-mono">{log.cost ? `$${log.cost.toFixed(2)}` : ''}</span>
                    </div>
                    {log.description && <p className="text-white/60 text-sm mb-3 whitespace-pre-wrap">{log.description}</p>}
                    <div className="flex flex-wrap gap-x-6 gap-y-2 mt-3 pt-3 border-t border-white/5 text-[11px] uppercase tracking-wider font-semibold">
                      <div className="text-white/30">Date: <span className="text-white/70">{new Date(log.service_date).toLocaleDateString()}</span></div>
                      <div className="text-white/30">Tech: <span className="text-white/70">{log.technicians ? `${log.technicians.first_name}` : 'Unknown'}</span></div>
                      {(log.next_due_date || log.next_due_miles_hours) && (
                        <div className="text-brand-orange/60 ml-auto">
                          Next Due: <span className="text-brand-orange">
