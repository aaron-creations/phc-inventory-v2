...
function RestockEntry({ entry, index, products, onUpdate, onRemove }) {
  ...
  return (
    <div className="glass rounded-xl p-4 relative">
      ...
      <input type="date" value={entry.date} onChange={e => onUpdate(index, 'date', e.target.value)}
      ...
      <select value={entry.productId} onChange={e => onUpdate(index, 'productId', e.target.value)}
      ...
      {selectedProduct && (
        <p className="text-white/30 text-xs mb-3 font-mono">Current stock: {selectedProduct.containers_in_stock.toFixed(2)} containers</p>
      )}
      <label className="text-white/40 text-xs mb-1 block">Containers Received</label>
      <input type="number" step="0.5" min="0" value={entry.containers} onChange={e => onUpdate(index, 'containers', e.target.value)}
        placeholder="e.g. 2"
        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm mb-3 outline-none focus:border-brand-orange/50" />
      {selectedProduct?.cost_per_container && entry.containers && parseFloat(entry.containers) > 0 && (
        <p className="text-brand-orange/70 text-xs mb-3 font-mono">Est. cost: ${(selectedProduct.cost_per_container * parseFloat(entry.containers)).toFixed(2)}</p>
      )}
      <label className="text-white/40 text-xs mb-1 block">Vendor <span className="text-white/20">(optional)</span></label>
      <input type="text" value={entry.vendor} onChange={e => onUpdate(index, 'vendor', e.target.value)}
      ...
      <input type="text" value={entry.invoiceNotes} onChange={e => onUpdate(index, 'invoiceNotes', e.target.value)}
